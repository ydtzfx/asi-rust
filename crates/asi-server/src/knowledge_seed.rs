...
pub fn seed_knowledge(kb: &KnowledgeBase) {
    ...
    kb.upsert(
        "project_name",
        "ASI-Rust — Enterprise AI Coding Assistant Platform",
    );
    kb.upsert(
        "project_type",
        "Rust workspace with 22 crates, Axum web server, Ollama/DeepSeek AI",
    );
    ...
    kb.upsert(
        "entrypoint",
        "crates/asi-server/src/main.rs — single binary entrypoint, binds port 3000",
    );
    kb.upsert(
        "chat_handler",
        "crates/asi-server/src/routes/chat.rs — main chat API handler with SSE streaming",
    );
    kb.upsert(
        "agent_loop",
        "crates/asi-ai-sdk/src/agent/tool_loop.rs — ToolLoopAgent core loop with tool calling",
    );
    ...
    kb.upsert(
        "path_safety",
        "All file ops use resolve_safe_path() with TOCTOU protection via verify_path_after_write()",
    );
    ...
    kb.upsert(
        "security_auth",
        "Clerk JWT with RS256. Dev-mode requires sk_test_ key + ASI_DEV_AUTH_BYPASS=true",
    );
