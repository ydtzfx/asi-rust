    ...
    pub fn summarize(&self) -> AnalyticsSummary {
        ...
        if points.is_empty() {
            return AnalyticsSummary {
                ...
                insights: vec!["No data collected yet".into()],
            };
        }
        ...
        let avg_latency: f64 =
            points.iter().map(|p| p.avg_latency_ms).sum::<f64>() / points.len() as f64;
        let peak = points
            .iter()
            .map(|p| p.requests as f64 / 60.0)
            .fold(0.0f64, f64::max);
        ...
        let first_half_rps: f64 = points[..mid.max(1)]
            .iter()
            .map(|p| p.requests as f64)
            .sum::<f64>()
            / mid.max(1) as f64;
        let second_half_rps: f64 = points[mid..].iter().map(|p| p.requests as f64).sum::<f64>()
            / (points.len() - mid).max(1) as f64;
        let trend = if second_half_rps > first_half_rps * 1.1 {
            TrendDirection::Improving
        } else if second_half_rps < first_half_rps * 0.9 {
        ...
        if error_rate > 0.05 {
            insights.push(format!("High error rate: {:.1}%", error_rate * 100.0));
        }
        if avg_latency > 5000.0 {
            insights.push(format!(
                "High latency: {:.0}ms avg — consider scaling",
                avg_latency
            ));
        }
        if trend == TrendDirection::Improving {
            insights.push("Usage trending upward — capacity planning recommended".into());
        }
        if total_tokens > 1_000_000 {
            insights.push("High token usage — review caching strategy".into());
        }

        AnalyticsSummary {
            period: format!("{} data points", points.len()),
            total_requests,
            total_tokens,
            total_errors,
            error_rate,
...
mod tests {
    ...
    fn test_record_and_summarize() {
        let lake = DataLake::new();
        for i in 0..10 {
            lake.record(UsagePoint {
                timestamp: i,
                tokens_used: 100,
                requests: 50,
                errors: if i == 5 { 5 } else { 0 },
                active_users: 3,
