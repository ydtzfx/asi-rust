...
impl ClosedLoop {
    pub fn new() -> Self {
        Self {
            ...
            iteration_count: std::sync::atomic::AtomicU64::new(0),
        }
    }
    ...
    pub async fn iterate(&self) -> LoopIteration {
        ...
        for outcome in &iter.outcomes {
            if outcome.success {
                iter.learnings.push(format!(
                    "✅ {} improved by {:.2}",
                    outcome.action_id, outcome.delta
                ));
            } else {
                iter.learnings
                    .push(format!("❌ {} had no effect", outcome.action_id));
            }
        }
    ...
    fn decide(&self, anomalies: &[&Observation]) -> Vec<Action> {
        ...
        for obs in anomalies {
            ...
            actions.push(Action {
                id: format!("act_{}", now()),
                action_type: at,
                target: obs.source.clone(),
                reason: format!("{} anomaly: {}", obs.metric, obs.value),
            });
        }
        if actions.is_empty() {
            actions.push(Action {
                ...
                target: "system".into(),
                reason: "All metrics nominal".into(),
            });
        }
        actions
    }
    ...
    pub fn start_continuous(controller: Arc<ClosedLoop>, interval_secs: u64) {
        tokio::spawn(async move {
            loop {
                ...
                tracing::info!(
                    "Loop iteration {} complete — {} learnings",
                    iter.id,
                    iter.learnings.len()
                );
