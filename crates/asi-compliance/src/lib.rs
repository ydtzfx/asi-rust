...
impl ComplianceEngine {
    pub fn new() -> Self {
        let controls = vec![
            ...
            Control {
                ...
                description: "Authentication required for protected endpoints".into(),
                status: ControlStatus::Compliant,
                evidence: vec![Evidence {
                    id: "e1".into(),
                    source: "asi-auth".into(),
                    description: "Clerk JWT middleware on all protected routes".into(),
                    collected_at: now(),
            ...
            Control {
                ...
                framework: Framework::SOC2,
                category: "Security".into(),
                description: "Rate limiting enabled".into(),
                status: ControlStatus::Compliant,
                evidence: vec![Evidence {
                    id: "e2".into(),
                    source: "middleware.rs".into(),
                    description: "GlobalRateLimitLayer: 60 req/min".into(),
                    collected_at: now(),
                    valid_until: now() + 86400 * 365,
                }],
            },
            ...
            Control {
                ...
                description: "JWT verification with algorithm validation".into(),
                status: ControlStatus::Compliant,
                evidence: vec![Evidence {
                    id: "e3".into(),
                    source: "asi-auth/src/clerk.rs".into(),
                    description: "typ=JWT + alg=RS256 validation".into(),
                    collected_at: now(),
                    valid_until: now() + 86400 * 365,
                }],
            },
            ...
            Control {
                ...
                description: "Backup and restore procedures".into(),
                status: ControlStatus::Compliant,
                evidence: vec![Evidence {
                    id: "e4".into(),
                    source: "scripts/backup.sh".into(),
                    description: "Automated backup with 7-day retention".into(),
                    collected_at: now(),
                    valid_until: now() + 86400 * 365,
                }],
            },
            ...
            Control {
                ...
                description: "Automated alerting and recovery".into(),
                status: ControlStatus::Compliant,
                evidence: vec![Evidence {
                    id: "e5".into(),
                    source: "asi-automation".into(),
                    description: "Watchdog + Self-heal engine + Prometheus alerts".into(),
                    collected_at: now(),
                    valid_until: now() + 86400 * 365,
                }],
            },
        ...
        let policies = vec![
            PolicyRule {
                ...
                description: "TLS must be configured for production".into(),
                check: PolicyCheck::TlsConfigured,
            },
            PolicyRule {
                id: "P02".into(),
                name: "Rate Limit Active".into(),
                description: "API rate limiting must be enabled".into(),
                check: PolicyCheck::RateLimitEnabled,
            },
            PolicyRule {
                id: "P03".into(),
                name: "Auth Key Set".into(),
                description: "CLERK_SECRET_KEY must be configured".into(),
                check: PolicyCheck::EnvVarSet("CLERK_SECRET_KEY".into()),
            },
        ];
        Self {
    ...
    pub fn audit(&self, framework: Framework) -> AuditReport {
        ...
        let non_compliant = controls
            .iter()
            .filter(|c| matches!(c.status, ControlStatus::NonCompliant(_)))
            .count();
        AuditReport {
            framework,
            generated_at: now(),
            ...
            findings: controls
                ...
                .filter_map(|c| match &c.status {
                    ControlStatus::NonCompliant(r) => Some(format!("{}: {}", c.id, r)),
                    _ => None,
                })
                .collect(),
            recommendations: if non_compliant > 0 {
                vec!["Address non-compliant controls before next audit".into()]
            } else {
                vec![]
            },
        }
