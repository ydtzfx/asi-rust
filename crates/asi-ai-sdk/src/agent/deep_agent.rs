...
impl DeepAgent {
    ...
    pub async fn execute(
        ...
        tokio::spawn(async move {
            let _ =
                run_deep_cycle(&provider, &task_owned, max_iter, &tx_clone, &cancel_clone).await;
        });
...
async fn run_deep_cycle(
    ...
    let _ = tx.send(AgentEvent::TextDelta {
        content: format!(
            "## Deep Agent: Plan → Execute → Reflect → Revise\n\n**Task:** {}\n\n",
            task
        ),
    });
    ...
    let _ = tx.send(AgentEvent::TextDelta {
        content: format!(
            "*Best reasoning path ({} selected nodes; tree {} nodes, avg score {:.2}):*\n",
            best_path.len(),
            tree.size(),
            tree.avg_score()
        ),
    ...
    while iteration < max_iter {
        ...
        if quality >= quality_threshold || iteration >= max_iter {
            let complete = quality >= quality_threshold;
            let _ = tx.send(AgentEvent::TextDelta {
                content: format!(
                    "\n### {}\n\n**Final output:**\n\n{}",
                    if complete {
...
mod tests {
    ...
    fn test_deep_stage_progression() {
        ...
        assert!(
            stages
                .windows(2)
                .all(|w| w[0] as u8 + 1 == w[1] as u8 || w[0] == DeepStage::Complete)
        );
    }
