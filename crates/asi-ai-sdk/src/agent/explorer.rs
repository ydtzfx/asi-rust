        ...
        loop {
            ...
            current = node
                ...
                .filter_map(|cid| {
                    self.nodes
                        .get(cid)
                        .map(|c| (cid.clone(), self.ucb1(c, node.visits)))
                })
...
pub fn score_thought(thought: &str) -> f64 {
    ...
    if thought.contains("therefore") || thought.contains("结论") {
        score += 0.15;
    }
    if thought.contains("because") || thought.contains("因为") {
        score += 0.1;
    }
    if thought.contains("step") || thought.contains("步骤") {
        score += 0.1;
    }
    if thought.len() > 50 {
        score += 0.1;
...
mod tests {
    ...
    fn test_thought_tree_branch_and_select() {
        ...
        assert!(tree.select_best_leaf().is_some());
