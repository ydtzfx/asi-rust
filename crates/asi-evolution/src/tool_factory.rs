...
impl ToolFactory {
    ...
    pub fn create_from_spec(&mut self, spec: &serde_json::Value) -> Result<String, String> {
        let name = spec["name"].as_str().ok_or("Missing 'name' in tool spec")?;
        let description = spec["description"].as_str().unwrap_or("Dynamic tool");
            ...
            match spec["handler_type"].as_str().unwrap_or("echo") {
                "http_get" => {
                    let url = spec["url"]
                        ...
                        .to_string();
                    Box::new(move |_| format!("HTTP GET {} → mock response", url))
                }
                "shell" => {
                    let cmd = spec["command"].as_str().unwrap_or("echo").to_string();
                    Box::new(move |_| format!("Executed: {}", cmd))
                }
                _ => {
                    let label = name.to_string();
                    Box::new(move |args| format!("[{}] received: {}", label, args))
                }
            };
...
mod tests {
    ...
    fn test_create_echo_tool() {
        let mut f = ToolFactory::new();
        let n = f
            .create_from_spec(&serde_json::json!({"name":"greet","description":"Greet the user"}))
            .unwrap();
        assert_eq!(n, "greet");
    ...
    fn test_create_multiple_tools() {
        let mut f = ToolFactory::new();
        f.create_from_spec(&serde_json::json!({"name":"search_docs","description":"Search documentation","handler_type":"http_get","url":"https://docs.example.com/search"})).unwrap();
        f.create_from_spec(&serde_json::json!({"name":"run_benchmark","description":"Run performance benchmark","handler_type":"shell","command":"cargo bench"})).unwrap();
