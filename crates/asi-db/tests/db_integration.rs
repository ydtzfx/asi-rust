...
async fn test_upsert_and_get_user() {
    ...
    let user = asi_db::queries::users::upsert_user(
        &pool, "user_1", "test@example.com", Some("Alice"), None, None, None,
    )
    .await
    .unwrap();
    ...
    let fetched = asi_db::queries::users::get_user(&pool, "user_1").await.unwrap();
...
async fn test_session_crud() {
    ...
    asi_db::queries::users::upsert_user(
        &pool, "user_1", "test@example.com", None, None, None, None,
    ).await.unwrap();

    let session = asi_db::queries::sessions::create_session(
        &pool, "sess_1", "user_1", Some("Test Session"),
    ).await.unwrap();
    assert_eq!(session.title.unwrap(), "Test Session");
    assert_eq!(session.message_count, 0);

    let updated = asi_db::queries::sessions::update_session(
        &pool, "sess_1", "user_1", None, Some("{\"key\":\"val\"}"), 10, 500,
    ).await.unwrap();
...
async fn test_session_cleanup() {
    ...
    asi_db::queries::sessions::create_session(
        &pool, "sess_old", "user_1", None,
    ).await.unwrap();

    // Set session timestamp to 8 days ago (well beyond 7-day cleanup)
    sqlx::query("UPDATE sessions SET updated_at = ? WHERE id = ?")
        .bind(std::time::SystemTime::now()
            .duration_since(std::time::UNIX_EPOCH).unwrap().as_secs() as i64 - 691200) // 8 days
        .bind("sess_old")
        .execute(&pool).await.unwrap();

    let deleted = asi_db::session_cleanup::clean_stale_sessions(&pool, 604800).await.unwrap();
    assert_eq!(deleted, 1);
}
...
async fn test_audit_log_insert() {
    ...
    asi_db::queries::audit::insert_audit_log(
        &pool, "user_1", "test_action", "Test summary", None, None, Some("127.0.0.1"),
    ).await.unwrap();
