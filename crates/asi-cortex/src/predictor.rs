...
impl CortexPredictor {
    ...
    pub fn predict(&self) -> Vec<Prediction> {
        ...
        if self.history.len() < 3 { return predictions; }
        ...
        if last_err > first_err * 2.0 {
            predictions.push(Prediction { subsystem: "server".into(), event: "Error rate doubling".into(), probability: (last_err / (first_err + 0.001) / 2.0).min(0.95), estimated_time_secs: 300, impact: ImpactLevel::Medium, recommended_action: "Review recent deployments for regressions".into() });
        }

        let pool = recent[2].database.metrics.iter().find(|m| m.key == "pool_active").map(|m| m.value).unwrap_or(0.0);
        if pool > 7.0 {
            predictions.push(Prediction { subsystem: "database".into(), event: "Connection pool near max".into(), probability: pool / 10.0, estimated_time_secs: 600, impact: ImpactLevel::High, recommended_action: "Increase DATABASE_POOL_SIZE or add connection pooling".into() });
        }
        ...
        if last_lat > first_lat * 3.0 && last_lat > 5000.0 {
            predictions.push(Prediction { subsystem: "ai".into(), event: "Provider latency spike".into(), probability: 0.8, estimated_time_secs: 120, impact: ImpactLevel::High, recommended_action: "Switch to fallback model or check provider health".into() });
        }
...
mod tests {
    ...
    fn test_feeds_and_predicts() {
        ...
        for _ in 0..5 { predictor.feed(monitor.snapshot()); }
